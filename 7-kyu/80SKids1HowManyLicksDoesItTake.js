...
const AVERAGE_LICKS = 252;

function totalLicks(env) {
    let toughChallengeName = "";
    let toughChallengeLicks = 0;
    let totalLicks = AVERAGE_LICKS;

    Object.entries(env).forEach(([challengeName, challengeLicks]) => {
        if (challengeLicks > 0 && challengeLicks > toughChallengeLicks) {
            toughChallengeName = challengeName;
            toughChallengeLicks = challengeLicks;
        }

        totalLicks += challengeLicks;
    });

    const toughChallengeString = toughChallengeName
        ? ` The toughest challenge was ${toughChallengeName}.`
        : "";

    return `It took ${totalLicks} licks to get to the tootsie roll center of a tootsie pop.${toughChallengeString}`;
}
