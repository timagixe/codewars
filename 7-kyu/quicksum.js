...
const INVALID_STRING_REGEXP = /[^A-Z\s]/;

function quicksum(packet) {
    if (INVALID_STRING_REGEXP.test(packet)) return 0;

    return [...packet].reduce((acc, cur, index) => {
        if (cur === " ") return acc;
        return acc + (index + 1) * (cur.charCodeAt() - "A".charCodeAt() + 1);
    }, 0);
}
